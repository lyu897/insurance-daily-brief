...
async function summarize(articles) {
  ...
  const prompt = `당신은 한국 보험산업 전문 에디터입니다. 아래 최근 24시간의 기사 메타데이터를 바탕으로 중복 기사를 하나의 이슈로 묶고, 가장 중요한 보험 이슈 최대 5개와 오늘의 핵심 키워드 3~5개를 골라 주세요. 또한 보험사 자산운용·상품가격·건전성에 영향을 주는 금융시장 변수(미국 금리, 국내외 채권금리, 환율, 부동산 등)를 2~3개 골라 보험시장과의 연결점을 설명하세요. marketSummary에는 오늘의 금융시장 흐름을 보험시장 관점에서 1문장으로 요약하세요. 기사 본문에 없는 사실은 절대 추가하지 마세요. 요약은 핵심 내용과 의미를 합쳐 2문장으로 작성하세요.\n\n기사 목록:\n${JSON.stringify(articles)}`;
  const schema = { type: "object", additionalProperties: false, required: ["editorNote", "keywords", "marketSummary", "marketContext", "issues"], properties: { editorNote: { type: "string" }, keywords: { type: "array", minItems: 3, maxItems: 5, items: { type: "string" } }, marketSummary: { type: "string" }, marketContext: { type: "array", minItems: 2, maxItems: 3, items: { type: "object", additionalProperties: false, required: ["label", "summary", "url", "source"], properties: { label: { type: "string" }, summary: { type: "string" }, url: { type: "string" }, source: { type: "string" } } } }, issues: { type: "array", maxItems: 5, items: { type: "object", additionalProperties: false, required: ["tag", "title", "summary", "url", "source"], properties: { tag: { type: "string" }, title: { type: "string" }, summary: { type: "string" }, url: { type: "string" }, source: { type: "string" } } } } } };
